...
<!-- AJAX -->
  <script>
            ...
            for (var i = 0; i < data.length; i++) {
                baris += `
                    <tr>
                        ...
                        <td>
                            <a onclick="hapusData(${data[i].id_gallery})" class="btn btn-sm btn-danger text-light">Hapus</a>
                        </td>
                    </tr>`;
            }
    ...
    $('#formUpload').off('submit').on('submit', function(e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                ...
                ambilData(); // Refresh data
                $("[name='gambar']").val(''); // Reset file input
            } else {
                swal({
                    title: "Gagal",
                    text: response.message,
